...
impl Elf32_Rel {
    pub fn read_bytes(bytes: &[u8], offset: usize) -> Self {
        let r_offset = u32::from_le_bytes(bytes[offset..offset+4].try_into().unwrap()) as Elf32_Addr;
...
impl Elf64_Rel {
    pub fn read_bytes(bytes: &[u8], offset: usize) -> Self {
        let r_offset = u64::from_le_bytes(bytes[offset..offset+8].try_into().unwrap()) as Elf64_Addr;
